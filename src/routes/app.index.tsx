...
function Page() {
  ...
  return (
    <AppShell>
      <PageHeader
        title={`Olá, ${me.data?.name.split(" ")[0] ?? "—"} 👋`}
        description="Como você está se sentindo hoje? Se quiser conversar, estamos aqui."
      />

      <div className="grid gap-4 sm:grid-cols-2">
        <Link to="/app/conversar" className="group relative overflow-hidden rounded-3xl bg-primary p-6 text-primary-foreground shadow-soft transition hover:opacity-95">
          ...
          <span className="mt-4 inline-block text-sm font-medium underline-offset-4 group-hover:underline">Entrar na fila →</span>
        </Link>

        <Link to="/app/historico" className="group rounded-3xl border bg-card p-6 shadow-soft transition hover:bg-muted/30">
          ...
          <span className="mt-4 inline-block text-sm font-medium text-primary underline-offset-4 group-hover:underline">Ver histórico →</span>
        </Link>
      </div>
          ...
          {(conv.data ?? []).slice(0, 4).map((c) => (
            <div key={c.id} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{fmtDate(c.startedAt)} · {fmtRelative(c.startedAt)}</p>
              </div>
      ...
      <section className="mt-10 rounded-2xl border bg-secondary/40 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Torne-se voluntário e ofereça escuta a outras pessoas.</p>
            <Link to="/app/candidatura" className="mt-3 inline-block">
              <Button variant="outline" size="sm">Quero ser voluntário</Button>
            </Link>
          </div>
        </div>
      </section>

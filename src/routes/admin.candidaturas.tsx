...
function Page() {
  ...
  const list = useMemo(() => (q.data ?? []).filter((a) => {
  ...
  return (
    <AppShell>
      ...
      <ul className="divide-y rounded-2xl border bg-card">
        {list.map((a) => (
          <li key={a.id}>
            <Link to="/admin/candidaturas/$id" params={{ id: a.id }} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-4 py-4 hover:bg-muted/40">
              <div className="min-w-0">
                <p className="truncate text-sm font-medium">{a.candidateAlias}</p>
                <p className="text-xs text-muted-foreground">Enviada {fmtRelative(a.submittedAt)} · {a.availability}</p>
              </div>
              <StatusBadge status={a.status} />
            </Link>
          </li>
        ))}
        {!q.isPending && list.length === 0 && <p className="p-6 text-center text-sm text-muted-foreground">Nenhuma candidatura.</p>}
      </ul>
    </AppShell>
  );
}
